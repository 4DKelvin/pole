...
SyntaxTree.prototype = {
    ...
    eachInputElement: function(fn, scope) {
        _.each(this.elements, fn, scope);
    }
};
...
var InputElement = function(options) {
    ...
    if (options && options.tokens) {
        _.each(_.isArray(options.tokens) ? options.tokens : [options.tokens], function(token) {
            me.addToken(token);
        });
    }
};
InputElement.prototype = {
    ...
    eachToken: function(fn, scope) {
        _.each(this.tokens, fn, scope);
    }
};
...
Token.prototype = {
    ...
    eachInputElement: SyntaxTree.prototype.eachInputElement
};
